...
export default function AuditPage() {
  ...
  const getActionIcon = (action: string) => {
    switch (action) {
      ...
      default: return <Edit size={14} className="text-blue-500" />
    }
  }
  ...
  return (
    <div className="page-enter">
      ...
      {loading ? (
        ...
        <EmptyState message="No audit logs yet." icon={<ScrollText size={48} />} />
      ) : (
                ...
                <tr key={log.id} className="stagger-item">
                  <td>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium capitalize" style={{ color: getActionColor(log.action) }}>
                        {log.action}
                      </span>
                    </div>
                  </td>
                  ...
                  <td>#{log.entityId}</td>
                  <td className="text-sm max-w-[300px] truncate" style={{ color: 'var(--muted-foreground)' }}>
                    {log.details || '-'}
                  </td>
